...
fun NavGraphBuilder.gamesNav(navController: NavController, width: Int) {
    composable(navCommand = NavCommand.ContentType(Features.GAMES), enterTransition = {
        slideInHorizontally(initialOffsetX = { width }) + fadeIn(
            animationSpec = tween(
                TWEEN_ANIM_DURATION
            )
        )
        ...
        GamesRoute {
            navController.navigate(
                NavCommand.ContentTypeDetail(Features.GAMES).createRoute(it.id)
            )
        }
    }
}